...
/*   Updated: 2023/05/19 18:08:30 by maderuel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include <fract_ol.h>
...
int	check_args(char **arg)
{
	if (!s_cmp(arg[1], "mandelbrot") || !s_cmp(arg[1], "m")
		|| !s_cmp(arg[1], "julia") || !s_cmp(arg[1], "j") || !s_cmp(arg[1], "zierpinski") || !s_cmp(arg[1], "z"))
		return (0);
	return (1);
}
...
void	render_img(t_fractal *f, t_pxl *p, t_mlx *mlx)
{
	...
	else if (!s_cmp(f->name, "zierpinski"))
		draw(&mlx->img, p, f, &zierpinski, &get_colors);
